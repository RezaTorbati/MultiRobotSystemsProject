...
# Edit log
# 10/2
    # (done) logic for picking zone/endpoint
    # (done) way to track/update agent reward
    # (done) way to visualize reward zone
    # (done) distinct color for agents and rewards
    # (done) print rewards
...
r = robotarium.Robotarium(goals=goal_points, number_of_robots=N, show_figure=True, initial_conditions=initial_conditions, sim_in_real_time=False)
...
while (np.size(at_pose(np.vstack((x_si,x[2,:])), goal_points, rotation_error=100)) != N):
    ...
    reward[at_pose(np.vstack((x_si,x[2,:])), goal_points, position_error=0.05, rotation_error=100)] += 1
    print(reward)
