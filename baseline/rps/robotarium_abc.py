...
class RobotariumABC(ABC):
    ...
    def __init__(self, goals, number_of_robots=-1, show_figure=True, sim_in_real_time=True, initial_conditions=np.array([])):
        ...
        if(self.show_figure):
            ...
            color = plt.cm.get_cmap('hsv', self.number_of_robots)
            ...
            for i in range(number_of_robots):
                ...
                p = patches.Rectangle((self.poses[:2, i]+self.robot_length/2*np.array((np.cos(self.poses[2, i]+math.pi/2), np.sin(self.poses[2, i]+math.pi/2)))+\
                                                0.04*np.array((-np.sin(self.poses[2, i]+math.pi/2), np.cos(self.poses[2, i]+math.pi/2)))), self.robot_length, self.robot_width, (self.poses[2, i] + math.pi/4) * 180/math.pi, 
                                                facecolor=color(i), edgecolor='k')
                # ^ old color = '#FFD700'
                ...
                goal = patches.Circle(xy=(goals[0][i], goals[1][i]), radius=0.05, facecolor="none", edgecolor=color(i))
